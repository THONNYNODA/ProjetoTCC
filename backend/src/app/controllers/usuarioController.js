...
router.put('/senha/:usuarioId', async (req,res) =>{

    try {
        const { dsSenha } = req.body

        const usuario = await Usuario.findByIdAndUpdate(req.params.usuarioId, {dsSenha}, {new: true}).select('+dsSenha') ;
